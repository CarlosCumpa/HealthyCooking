...
public class RegistroController {
    ...
    public ResponseEntity<Registro> delete(@PathVariable(value = "id")Long id) throws Exception {
        Registro deleteRegistro = registroService.delete(id);
        return new ResponseEntity<Registro>(registroService.delete(id), HttpStatus.OK);
    }
    @GetMapping("/registros/{prefijo}")
    public ResponseEntity<List<Registro>> listFirstname(@PathVariable(value = "prefijo")String prefijo){
        List<Registro> registroBuscar = registroService.listFirstName(prefijo);
        return new ResponseEntity<List<Registro>>(registroService.listFirstName(prefijo),HttpStatus.OK);
    }
